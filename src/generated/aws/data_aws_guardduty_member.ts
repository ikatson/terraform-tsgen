...
export interface data_aws_guardduty_member_config {
    account_id: rstring
    detector_id: rstring
...
export class data_aws_guardduty_member implements Data {
    config: data_aws_guardduty_member_config
    private _is_data: boolean = true
    private _name: string = "data_aws_guardduty_member"
    private _id: string;
    constructor(id: string, config: data_aws_guardduty_member_config) {
        this._id = id;
        this.config = config;
    }
    getID(): string {
    ...
    get ref_disable_email_notification(): Reference<boolean> {
        return new Reference(`data.${this._name}.${this._id}.disable_email_notification`)
  }
    ...
    get ref_invitation_message(): Reference<string> {
        return new Reference(`data.${this._name}.${this._id}.invitation_message`)
  }
    get ref_invite(): Reference<boolean> {
        return new Reference(`data.${this._name}.${this._id}.invite`)
  }
    get ref_relationship_status(): Reference<string> {
        return new Reference(`data.${this._name}.${this._id}.relationship_status`)
  }
}
