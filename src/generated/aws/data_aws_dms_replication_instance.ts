...
export interface data_aws_dms_replication_instance_config {
...
export class data_aws_dms_replication_instance implements Data {
    config: data_aws_dms_replication_instance_config
    private _is_data: boolean = true
    private _name: string = "data_aws_dms_replication_instance"
    private _id: string;
    constructor(id: string, config: data_aws_dms_replication_instance_config) {
        this._id = id;
        this.config = config;
    }
    getID(): string {
    ...
    get ref_replication_instance_class(): Reference<string> {
        return new Reference(`data.${this._name}.${this._id}.replication_instance_class`)
  }
    get ref_replication_instance_id(): Reference<string> {
        return new Reference(`data.${this._name}.${this._id}.replication_instance_id`)
  }
    get ref_replication_instance_private_ips(): Reference<rstring[]> {
        return new Reference(`data.${this._name}.${this._id}.replication_instance_private_ips`)
  }
    get ref_replication_instance_public_ips(): Reference<rstring[]> {
        return new Reference(`data.${this._name}.${this._id}.replication_instance_public_ips`)
  }
