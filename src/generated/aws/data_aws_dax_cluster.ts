...
export interface _data_aws_dax_cluster__nodes {
    address?: rstring
...
export interface _data_aws_dax_cluster__server_side_encryption {
    enabled?: rboolean
}

export interface data_aws_dax_cluster_config {
    arn?: rstring
    ...
    node_type: rstring
    nodes?: _data_aws_dax_cluster__nodes[]
    notification_topic_arn?: rstring
    parameter_group_name?: rstring
    port?: rnumber
    replication_factor: rnumber
    security_group_ids?: rstring[]
    server_side_encryption?: _data_aws_dax_cluster__server_side_encryption[]
    subnet_group_name?: rstring
    tags?: {[propName: string]: rstring}
}

export class data_aws_dax_cluster implements Data {
    config: data_aws_dax_cluster_config
    private _is_data: boolean = true
    private _name: string = "data_aws_dax_cluster"
    private _id: string;
    constructor(id: string, config: data_aws_dax_cluster_config) {
        this._id = id;
        this.config = config;
    }
    getID(): string {
    ...
    get ref_nodes(): Reference<_data_aws_dax_cluster__nodes[]> {
        return new Reference(`data.${this._name}.${this._id}.nodes`)
  }
    ...
    get ref_replication_factor(): Reference<number> {
        return new Reference(`data.${this._name}.${this._id}.replication_factor`)
  }
    get ref_security_group_ids(): Reference<rstring[]> {
        return new Reference(`data.${this._name}.${this._id}.security_group_ids`)
  }
    get ref_server_side_encryption(): Reference<_data_aws_dax_cluster__server_side_encryption[]> {
